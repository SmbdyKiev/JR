   ...
   public static final int MAX_KEY_VALUE = 33;
    ...
    public String crypt(String source, CryptKey key){
        ...
        for (int i = 0; i < srcArray.length; i++) {
            if (srcArray[i]+key.getIntKey()> Character.MAX_VALUE) {
                resultString += (char)(srcArray[i]+key.getIntKey()- Character.MAX_VALUE);
            }
            else if (srcArray[i]+key.getIntKey()< Character.MIN_VALUE) {
                    resultString += (char)(srcArray[i]+key.getIntKey()+Character.MAX_VALUE);
                }
